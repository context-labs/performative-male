          ...
          {entries.map((e, i) => (
            <article key={e.id} className="rounded-lg border">
              <div className="relative h-52 sm:h-60 overflow-hidden rounded-t-lg bg-muted">
                <ExpandableImage src={`/img/${e.imageId}/${e.imageHash ?? 'x'}?w=640&fmt=webp&q=72`} alt={`Entry ${e.id}`} loading={i < 6 ? 'eager' : 'lazy'} fetchPriority={i < 6 ? 'high' : 'auto'} className="h-full w-full object-contain" />
